...
import { LoopState, SearchFilterConfig } from '@/src/types';
    ...
    <div className={`backdrop-blur-xl bg-white/80 dark:bg-slate-900/80 border rounded-2xl p-4 md:p-5 space-y-4 transition-all duration-500 shadow-lg relative overflow-hidden ${
      ...
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-1.5">
          ...
          {/* 9. Auto Engine Rotation */}
          <button
            type="button"
            onClick={onToggleAutoEngineRotation}
            className={`w-full py-1 px-2 rounded-lg text-[9px] font-black border cursor-pointer transition-all flex items-center justify-center gap-1.5 truncate uppercase tracking-tighter h-8 ${
              autoEngineRotationEnabled
                ? 'bg-fuchsia-600 text-white border-fuchsia-700 shadow-sm'
                : 'bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-400 hover:border-fuchsia-500'
            }`}
            title="Muda automaticamente o motor de busca caso encontre bloqueios (429)."
          >
            <RefreshCw className="w-3 h-3 shrink-0" />
            <span>{autoEngineRotationEnabled ? 'Auto Rotação On' : 'Auto Rotação Off'}</span>
          </button>
          
          {/* 10. AI Search Expansion (Smart Brain) */}
          <button
            type="button"
