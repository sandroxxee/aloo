      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl px-6 py-4 flex flex-wrap items-center justify-between gap-6 shadow-sm">
        <div className="flex items-center gap-4">
          ...
          <div className="flex flex-col">
            <span className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em]">Data Integrity Active</span>
            <span className="text-xs font-black text-slate-900 dark:text-slate-100 flex items-center gap-2">
              <span className="w-1.5 h-1.5 rounded-full bg-emerald-500" />
              LAST SNAPSHOT: {lastBackup || 'INITIALIZING...'}
            </span>
          </div>
        </div>
        ...
        <AnimatePresence mode="popLayout">
          {loading ? (
            ...
            insights.map((insight, idx) => (
              ...
              >
                ...
                <div className={`mt-6 pt-6 border-t flex items-center justify-between relative z-10 ${
                  ...
                  <div className="flex flex-col">
                    <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">IA Strategic</span>
                    <span className={`text-[10px] font-black ${insight.priority === 'ALTA' ? 'text-amber-500' : 'text-blue-600 dark:text-blue-400'}`}>VERIFIED</span>
                  </div>
            ...
            <div className="col-span-3 py-20 flex flex-col items-center justify-center bg-slate-50 dark:bg-slate-950/20 rounded-2xl border-2 border-dashed border-slate-200 dark:border-slate-800">
              ...
              <p className="text-slate-900 dark:text-white text-sm font-black uppercase tracking-widest mb-2">Synthetic Intelligence Processing</p>
              <p className="text-slate-500 text-xs font-medium max-w-xs text-center leading-relaxed">System requires a minimum of 5 verified lead data points to synthesize strategic commercial insights.</p>
