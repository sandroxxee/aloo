  ...
  return (
    <div className="flex flex-col h-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-sm overflow-hidden text-slate-900 dark:text-slate-100">
          ...
          <div className="flex items-center gap-6">
            ...
            <div>
              <div className="flex items-center gap-3">
                <h3 className="text-2xl font-black tracking-tighter text-slate-900 dark:text-white uppercase">Asset Intelligence Vault</h3>
              ...
              <p className="text-[11px] font-bold text-slate-500 uppercase tracking-[0.2em] mt-1.5 flex items-center gap-2.5">
                ...
                {filteredLeads.length.toLocaleString('pt-BR')} Verified Strategic Assets Mapped
              </p>
            </div>
          </div>
              ...
              {isMoreActionsOpen && (
                <div className="absolute right-0 mt-2 w-56 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-2xl p-2 z-[60] animate-in fade-in slide-in-from-top-2">
                  ...
                  <button onClick={() => { downloadVcfContacts(leads); setIsMoreActionsOpen(false); }} className="w-full flex items-center gap-3 px-3 py-2 text-xs font-bold text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 rounded-xl transition-colors">
                    <Share2 className="w-4 h-4 text-purple-500" /> WhatsApp Contacts (.vcf)
                  </button>
                  {onOpenImportModal && (
                    <button onClick={() => { onOpenImportModal(); setIsMoreActionsOpen(false); }} className="w-full flex items-center gap-3 px-3 py-2 text-xs font-bold text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 rounded-xl transition-colors">
                      <Upload className="w-4 h-4 text-indigo-500" /> Bulk Import Leads
                    </button>
                  )}
                  <div className="h-px bg-slate-100 dark:bg-slate-800 my-1" />
                  <button onClick={() => { onCleanLeads(); setIsMoreActionsOpen(false); }} className="w-full flex items-center gap-3 px-3 py-2 text-xs font-bold text-amber-600 hover:bg-amber-50 dark:hover:bg-amber-900/20 rounded-xl transition-colors">
                    <RefreshCw className="w-4 h-4" /> Purge Invalid Leads
                  </button>
      ...
      <div className="px-8 py-5 border-b border-slate-100 dark:border-slate-800/40 flex items-center gap-3 overflow-x-auto no-scrollbar bg-slate-50/30 dark:bg-slate-900/10">
        {[
          { id: 'ALL', label: 'All Intelligence', icon: Database, color: 'text-blue-500' },
      ...
      <div className="px-8 py-6 bg-white dark:bg-slate-900 border-b border-slate-100 dark:border-slate-800/40 flex flex-col md:flex-row items-center gap-6">
        <div className="relative flex-1 group">
          ...
          <input
            type="text"
            placeholder="Search Intelligence... (Name, Phone, Item, CNPJ)"
            value={searchTerm}
      ...
      <div className="px-6 py-4 bg-slate-50/50 dark:bg-slate-900/50 border-b border-slate-100 dark:border-slate-800/60 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="space-y-1.5">
          <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-1">Engagement Channel</label>
          <select
            value={channelFilter}
          ...
          >
            ...
            <option value="FIXO">Landline / Business</option>
            <option value="EMAIL">Email Marketing</option>
            <option value="LOJISTA">Fleet Owners / Dealers</option>
            <option value="PARTICULAR">Private Sellers</option>
          </select>
        </div>

        {/* Tag Intelligence Filter */}
        <div className="space-y-1.5">
          <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-1">Lead Tag Intelligence</label>
          <select
            value={tagFilter}
          ...
          >
            ...
            <option value="NO_TAGS">Untagged Leads ({leadsWithoutTagsCount})</option>
        ...
        {/* Commercial Intent */}
        <div className="space-y-1.5">
          <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-1">Commercial Intent</label>
          <select
            value={intentFilter}
          ...
          >
            ...
            <option value="Venda">ASSET DISPOSAL (VENDA)</option>
            <option value="Compra">ASSET ACQUISITION (COMPRA)</option>
            <option value="Troca">ASSET EXCHANGE (TROCA)</option>
          </select>
        </div>

        {/* Lead Status */}
        <div className="space-y-1.5">
          <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-1">Pipeline Status</label>
          <select
            value={statusFilter}
          ...
          >
            ...
            <option value="NOVO">NEW INTELLIGENCE</option>
            <option value="CONTATO">ENGAGEMENT ACTIVE</option>
            <option value="NEGOCIACAO">IN NEGOTIATION</option>
            <option value="GANHO">CONVERTED / CLOSED</option>
          </select>
        </div>
      </div>
      ...
      <div className="px-6 py-4 bg-white dark:bg-slate-900 border-b border-slate-100 dark:border-slate-800/60 flex flex-wrap items-center justify-between gap-4">
        <div className="flex items-center gap-3 overflow-x-auto no-scrollbar pb-1 sm:pb-0">
          <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest shrink-0">Priority Radar:</span>
        ...
        <div className="flex items-center gap-4 shrink-0">
          <div className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-50 dark:bg-slate-800 rounded-xl border border-slate-100 dark:border-slate-700">
             <span className="w-2 h-2 rounded-full bg-emerald-500" />
             <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Pipeline Health: Stable</span>
          </div>
        ...
        <div className="bg-slate-50 dark:bg-slate-800/50 border-b border-slate-200 dark:border-slate-700 px-6 py-3 flex items-center text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em]">
          ...
          <div className="flex-1 grid grid-cols-12 gap-4 items-center">
            <div className="col-span-5 flex items-center gap-2">
              ...
              PRIMARY ASSET INTELLIGENCE
            </div>
            <div className="col-span-2">LOGISTICS / REGION</div>
            <div className="col-span-2">ENGAGEMENT</div>
            <div className="col-span-2 text-center">INTENT & SCORE</div>
      ...
      <div className="px-8 py-6 border-t border-slate-200 dark:border-slate-800 flex flex-col sm:flex-row items-center justify-between gap-6 bg-slate-50/50 dark:bg-black text-[10px] font-black uppercase tracking-widest">
        <div className="flex flex-wrap items-center gap-6">
          <div className="flex items-center gap-3">
            <span className="text-slate-400">Vault Visibility</span>
