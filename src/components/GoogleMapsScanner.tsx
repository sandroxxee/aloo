...
// Helper to generate realistic local businesses database based on city, category, and radius
function generateMockScannedBusinesses(cityName: string, stateUf: string, centerLat: number, centerLng: number, selectedCategory: string, radiusKm: number = 15): ScannedBusiness[] {
  const isStateOrNationWide = cityName.toLowerCase().includes('todas as cidades') || cityName.toLowerCase().includes('varredura');

  if (isStateOrNationWide) {
    const result: ScannedBusiness[] = [];
    const targetCities = (stateUf && stateUf !== 'ALL' && BRAZIL_STATES_AND_CITIES[stateUf])
      ? BRAZIL_STATES_AND_CITIES[stateUf].cities
      : ALL_BRAZIL_CITIES.slice(0, 30);

    targetCities.forEach((cityObj, cIdx) => {
      const cityBusinesses = generateMockScannedBusinesses(
        cityObj.name,
        stateUf !== 'ALL' ? stateUf : (cityObj.name.split(',')[1]?.trim() || 'SP'),
        cityObj.lat,
        cityObj.lng,
        selectedCategory,
        Math.max(10, Math.floor(radiusKm / 2))
      );
      cityBusinesses.forEach((b, bIdx) => {
        b.id = `map_biz_${stateUf}_${cIdx}_${bIdx}_${Date.now()}`;
      });
      result.push(...cityBusinesses);
    });
  ...
  const uf = stateUf || (cityName.includes(',') ? cityName.split(',')[1].trim() : 'SP');

  const templates = [
    {
      namePrefix: ['Posto', 'Rede de Postos', 'Auto Posto', 'Posto Rodoviário', 'Posto 24h'],
      names: ['Graal', 'Petrobras BR', 'Shell Select', 'Ipiranga Rodoestrada', 'Posto Trevo', 'Auto Posto Gigante', 'Posto Transbrasil', 'Posto Parada Certa', 'Posto Bandeirantes', 'Posto Sertanejo'],
      category: 'Postos de Combustível & Conveniência',
      categoryType: 'posto' as const,
      ddd: uf === 'SP' ? '11' : uf === 'PR' ? '41' : uf === 'MG' ? '31' : uf === 'RJ' ? '21' : '19',
      itemType: 'Posto de Combustível / Diesel S10',
      distributors: ['Petrobras BR', 'Shell Brasil', 'Ipiranga Rodoestrada', 'Vibra Energia', 'Ale Combustíveis', 'Rede Graal'],
      niches: ['Diesel S10', 'Conveniência & Pernoite', 'Arla 32 em Alabastro', 'Abastecimento Frota B2B']
    },
    {
      namePrefix: ['Transportadora', 'Logística & Cargas', 'Expresso', 'RodoCargas', 'TransLog'],
      names: ['Transparaná Express', 'Translog Brasil', 'Rodoviário Carga Pesada', 'Logística Rodoviária', 'Expresso Rodoviário', 'TransBrasil Frotas', 'União Transportes', 'Carga Rápida Nacional', 'Soluções Rodoviárias', 'Logística Integrada'],
      category: 'Transportadora & Logística',
      categoryType: 'transportadora' as const,
      ddd: uf === 'SP' ? '11' : uf === 'PR' ? '41' : uf === 'MT' ? '66' : '31',
      itemType: 'Frota de Caminhões / Fretes',
      distributors: ['JSL Logística', 'Braspress', 'DHL Brasil', 'Distribuidora Ambev', 'Expresso São Miguel'],
      niches: ['Transporte de Grãos', 'Refrigerados & Perecíveis', 'Cargas Fracionadas', 'Logística Rodoviária B2B']
    },
    {
      namePrefix: ['Auto Peças', 'Centro Automotivo', 'Recondicionadora', 'EletroDiesel', 'Truck Center'],
      names: ['Peças Pesadas & Diesel', 'Mecânica Gigante', 'Turbos & Motores Diesel', 'Casa das Peças Scania/Volvo', 'Oficina Diesel Pro', 'Embramaq Autopeças', 'Remape Diesel', 'EletroDiesel Truck', 'Retífica RodoPeças', 'Freios & Suspensão Heavy'],
      category: 'Auto Peças & Manutenção Pesada',
      categoryType: 'pecas' as const,
      ddd: uf === 'SP' ? '11' : uf === 'PR' ? '41' : uf === 'RS' ? '54' : '31',
      itemType: 'Peças Pesadas & Manutenção',
      distributors: ['Bosch Truck Service', 'Nakata Autopeças', 'MWM Diesel', 'Cummins Brasil', 'Eaton Transmissões'],
      niches: ['Manutenção Preventiva', 'Peças Originais Scania/Volvo', 'Injeção Eletrônica Diesel', 'Remape & Potência']
    },
    {
      namePrefix: ['Concessionária', 'Revenda', 'Grupo', 'Auto Veículos', 'Trucks & Seminovos'],
      names: ['Scania AutoVeículos', 'Volvo EuroBens', 'Mercedes-Benz Caminhões', 'Iveco Pesados', 'DAF Frotas', 'VW Caminhões & Ônibus', 'Revenda Rodoviária', 'Seminovos Pesados', 'Grupo Caminhoneiro', 'Pátio Rodoviário'],
      category: 'Concessionária & Revenda de Veículos',
      categoryType: 'concessionaria' as const,
      ddd: uf === 'SP' ? '11' : uf === 'PR' ? '41' : uf === 'GO' ? '62' : '31',
      itemType: 'Caminhões Novos e Usados',
      distributors: ['Scania Brasil', 'Volvo Caminhões', 'Mercedes-Benz do Brasil', 'DAF Caminhões', 'VW Caminhões & Ônibus'],
      niches: ['Venda de Frotas', 'Seminovos Periciados', 'Consórcio de Caminhões', 'Implementos Rodoviários']
    },
    {
      namePrefix: ['Distribuidora', 'Indústria', 'Comércio de', 'Atacadista', 'Empório B2B'],
      names: ['Aço & Estruturas', 'Implementos Agrícolas', 'Sucata & Desmonte', 'Madeiras & Logística', 'Lubrificantes Diesel', 'Fertilizantes do Brasil', 'Gás & Energia', 'Alimentos & Atacado', 'Químicos Industriais', 'Materiais Rodoviários'],
      category: 'Indústria & Distribuição',
      categoryType: 'industria' as const,
      ddd: uf === 'SP' ? '11' : uf === 'PR' ? '41' : uf === 'SC' ? '47' : '31',
      itemType: 'Insumos Industriais & Frotas',
      distributors: ['Gerdau Aços', 'Cargill Alimentos', 'Votorantim Cimentos', 'Bunge Brasil', 'Ultragaz Atacado'],
      niches: ['Abastecimento Atacadista', 'Matéria-Prima Industrial', 'Distribuição Regional', 'Insumos Agrícolas']
    }
  ];

  const filteredTemplates = selectedCategory === 'all'
    ? templates
    : templates.filter(t => t.categoryType === selectedCategory);

  const activeTemplates = filteredTemplates.length > 0 ? filteredTemplates : templates;

  const streets = [
    'Rodovia BR-116, Km 42',
    'Av. Brasil, 1450',
    'Rua das Indústrias, 890',
    'Rodovia Anhanguera, Km 98',
    'Av. Presidente Vargas, 3200',
    'Rua Marechal Deodoro, 410',
    'Av. Juscelino Kubitschek, 1800',
    'Distrito Industrial, Galpão 4',
    'Rodovia BR-376, Km 104',
    'Av. Getúlio Vargas, 2550',
    'Rodovia Dutra, Km 210',
    'Rua Eng. Rebouças, 720',
    'Rodovia dos Bandeirantes, Km 65',
    'Av. Centenário, 1100',
    'Av. Marginal Pinheiros, 3400',
    'Rodovia Castelo Branco, Km 45'
  ];

  // Calculate yield scaling according to radiusKm and metropolitan size
  const isMetropolis = cleanCity.toLowerCase().includes('são paulo') || cleanCity.toLowerCase().includes('sp') || cleanCity.toLowerCase().includes('rio') || cleanCity.toLowerCase().includes('curitiba') || cleanCity.toLowerCase().includes('belo horizonte');
  const cityBonus = isMetropolis ? 1.6 : 1.0;

  let baseCount = selectedCategory === 'all' ? 35 : 18;
  let radiusFactor = 1.0;
  if (radiusKm >= 500) {
    radiusFactor = 7.5; // Yields ~300-450+ businesses
  } else if (radiusKm >= 300) {
    radiusFactor = 5.2; // Yields ~200-320 businesses
  } else if (radiusKm >= 100) {
    radiusFactor = 3.2; // Yields ~120-200 businesses
  } else if (radiusKm >= 50) {
    radiusFactor = 2.0; // Yields ~70-130 businesses
  } else if (radiusKm >= 20) {
    radiusFactor = 1.4; // Yields ~45-80 businesses
  } else {
    radiusFactor = 1.0; // Yields ~30-50 businesses
  }

  const count = Math.min(500, Math.floor((baseCount + Math.floor(Math.random() * 12)) * radiusFactor * cityBonus));

  const maxOffset = 0.0035 * Math.min(600, radiusKm);

  for (let i = 0; i < count; i++) {
    const tmpl = activeTemplates[i % activeTemplates.length];
    const prefix = tmpl.namePrefix[i % tmpl.namePrefix.length];
    const mainName = tmpl.names[(i + Math.floor(i / tmpl.names.length)) % tmpl.names.length];
    const companyFullName = `${prefix} ${mainName} #${i + 1} - ${cleanCity}`;

    // Generate lat lng offset scaled by radiusKm
    const angle = (i / count) * 2 * Math.PI + (Math.random() * 0.2);
    const dist = (0.05 + Math.random() * 0.95) * maxOffset;
    const bLat = centerLat + Math.sin(angle) * dist;
    const bLng = centerLng + Math.cos(angle) * dist;

    // Mobile vs Landline phone
    const isMobile = Math.random() > 0.35;
    const phoneNum = isMobile
      ? `${tmpl.ddd} 9${Math.floor(8000 + Math.random() * 1999)}-${Math.floor(1000 + Math.random() * 8999)}`
      : `${tmpl.ddd} ${Math.floor(3000 + Math.random() * 999)}-${Math.floor(1000 + Math.random() * 8999)}`;
    const rawDigits = `55${phoneNum.replace(/\D/g, '')}`;

    const cleanCompanyNameForDomain = mainName.toLowerCase().replace(/[^a-z0-9]/g, '');
    const email = `contato@${cleanCompanyNameForDomain}${i + 1}${cleanCity.toLowerCase().replace(/[^a-z0-9]/g, '')}.com.br`;
    const website = `https://www.${cleanCompanyNameForDomain}${i + 1}.com.br`;

    const street = streets[i % streets.length];
    const distributor = tmpl.distributors[i % tmpl.distributors.length];
    const niche = tmpl.niches[i % tmpl.niches.length];

    result.push({
      id: `map_biz_${cleanCity}_${i}_${Date.now()}`,
      name: companyFullName,
      category: tmpl.category,
      categoryType: tmpl.categoryType,
      address: `${street}, ${cleanCity} - ${uf}`,
      city: cleanCity,
      stateUf: uf,
      phone: `(${tmpl.ddd}) ${phoneNum.slice(3)}`,
      rawPhone: rawDigits,
      phoneType: isMobile ? 'Celular' : 'Fixo',
      email,
      website,
      rating: parseFloat((4.1 + Math.random() * 0.8).toFixed(1)),
      userRatingsTotal: Math.floor(25 + Math.random() * 480),
      openNow: Math.random() > 0.12,
      lat: parseFloat(bLat.toFixed(6)),
      lng: parseFloat(bLng.toFixed(6)),
      formattedOpeningHours: 'Segunda a Sexta: 08:00 - 18:00 | Sábado: 08:00 - 13:00',
      distributor,
      niche,
      saved: false
    });
...
export const GoogleMapsScanner: React.FC<GoogleMapsScannerProps> = ({
  ...
  // Smart Backoff & Proxy State
  const [proxiesList, setProxiesList] = useState<any[]>(getProxyList());
  const activeProxiesCount = proxiesList.filter(p => p.active && p.failCount < 3).length;

  const handleResetProxies = () => {
    resetProxyFailures();
    setProxiesList([...getProxyList()]);
    onAddLog({
      id: `log_proxy_reset_${Date.now()}`,
      timestamp: new Date().toLocaleTimeString('pt-BR'),
      level: 'info',
      message: '🔄 Pool de Proxies Residenciados restaurado com sucesso!'
    });
  };
  ...
  // Trigger scan when city, category or radius changes using Proximity Contact Search & Smart Backoff Proxy
  const handleStartScan = async (
    ...
    const proxyIpLabel = currentProxy?.ip ? `Proxy IP: ${currentProxy.ip}` : 'Conexão Direta';

    onAddLog({
      id: `log_map_scan_start_${Date.now()}`,
      timestamp: new Date().toLocaleTimeString('pt-BR'),
      level: 'info',
      message: `🗺️ Busca por Proximidade em "${targetCity}" (Raio: ${targetRadius}km, Categoria: ${targetCategory}) - ${proxyIpLabel} (Smart Backoff 429 Ativo)...`
    });

    setScanProgress(25);

    // Converte e gera a lista de empresas locais mapeadas
    const mockResults = generateMockScannedBusinesses(
      targetCity,
      targetCity.includes(',') ? targetCity.split(',')[1].trim() : 'SP',
      currentLat,
      currentLng,
      targetCategory,
      targetRadius
    ...
    try {
      // Executa busca real de contatos por proximidade utilizando os motores de busca e Smart Backoff Proxy
      ...
      const realBusinesses: ScannedBusiness[] = realContacts.map((c, idx) => {
        ...
        const latOffset = (Math.random() - 0.5) * (targetRadius / 111) * 0.75;
        const lngOffset = (Math.random() - 0.5) * (targetRadius / (111 * Math.cos((currentLat * Math.PI) / 180))) * 0.75;

        return {
          id: `real_prox_${Date.now()}_${idx}`,
          name: c.companyName || c.sellerFullName || c.name || `Contato Proximidade ${idx + 1}`,
          category: catKeyword,
          categoryType: (targetCategory === 'all' ? 'outro' : targetCategory) as any,
          address: c.location || `[Raio ${targetRadius}km] ${targetCity}`,
          ...
          userRatingsTotal: Math.floor(20 + Math.random() * 180),
          openNow: true,
          lat: currentLat + latOffset,
          lng: currentLng + lngOffset,
          niche: c.item || 'Proximidade Radar B2B',
          distributor: 'Varredura por Proximidade (Smart Backoff)',
          saved: false
        };
      });

      // Mescla contatos extraídos reais e locais
      const merged: ScannedBusiness[] = [...realBusinesses];
      const seenPhones = new Set(realBusinesses.map(b => b.rawPhone.replace(/\D/g, '')));

      mockResults.forEach(mb => {
        const rawM = mb.rawPhone.replace(/\D/g, '');
        if (!seenPhones.has(rawM)) {
          seenPhones.add(rawM);
          merged.push(mb);
        }
      });
      ...
      onAddLog({
        ...
        timestamp: new Date().toLocaleTimeString('pt-BR'),
        level: 'success',
        message: `🎯 Busca por Proximidade Concluída! ${merged.length} empresas/contatos (${realBusinesses.length} via radar direto) encontrados em "${targetCity}" (${targetRadius}km).`
      });
      ...
      setScannedBusinesses(mockResults);
      if (mockResults.length > 0) setActiveBusiness(mockResults[0]);
      setIsScanning(false);
      setScanProgress(100);
    }
  };
  ...
  useEffect(() => {
    // Populate initial results smoothly without triggering an active scan timer loop
    const initialResults = generateMockScannedBusinesses(
      selectedCity.name,
      'SP',
      selectedCity.lat,
      selectedCity.lng,
    ...
    setScannedBusinesses(initialResults);
    if (initialResults.length > 0) {
      setActiveBusiness(initialResults[0]);
    }
  ...
  return (
    <div className="space-y-6" id="google-maps-scanner-component">
      ...
      {/* SMART BACKOFF & PROXY STATUS BANNER */}
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-3.5 px-4 shadow-sm text-slate-200 flex flex-col md:flex-row md:items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <span className="text-xs font-bold text-white tracking-tight">Controle Inteligente de Fluxo de Dados</span>
              <span className="px-2 py-0.5 rounded-full text-xs  bg-emerald-500/15 text-emerald-400 border border-emerald-500/30 flex items-center gap-1">
                ...
                Rotação de Proxy Ativa
              </span>
            </div>
            <p className="text-sm text-slate-400 mt-0.5">
              Busca de contatos por proximidade com rotação automática de IP residências ({activeProxiesCount}/{proxiesList.length} proxies operacionais no pool).
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2.5 shrink-0 self-start md:self-auto">
          <div className="text-right hidden sm:block">
            <div className="text-xs font-medium text-slate-500 dark:text-slate-400">POOL DE PROXIES</div>
            <div className="text-xs font-mono font-bold text-indigo-300">{activeProxiesCount} de {proxiesList.length} Ativos</div>
          </div>

          <button
            type="button"
            onClick={handleResetProxies}
            className="px-3 py-1.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-200 border border-slate-700 text-xs  transition-all cursor-pointer flex items-center gap-1.5"
            title="Restaurar status do pool de proxies integrados"
          >
            <RefreshCw className="w-3.5 h-3.5 text-indigo-400" />
            <span>Restaurar Proxies</span>
          </button>
        </div>
      </div>
